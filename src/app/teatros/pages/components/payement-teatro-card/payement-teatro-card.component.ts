...
@Component({
  selector: 'app-payement-teatro-card',
  templateUrl: './payement-teatro-card.component.html',
  styleUrls: ['./payement-teatro-card.component.css'],
})
export class PayementTeatroCardComponent {
  myForm!: FormGroup;
    ...
    this.reservacionService.createReservacion(reservacion).subscribe({
      next: (response) => {
        ...
        Swal.fire({
          ...
          this.router.navigate(['/teatros/home']);
        });
      },
      error: (error) => {
        console.error(error);
